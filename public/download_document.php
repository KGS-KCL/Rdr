...
if (!User::hasRole(['Süper Admin', 'ISG Uzmanı'])) {
    http_response_code(403); die('Yetkiniz yok.');
}

if (!isset($_GET['id']) || !filter_var($_GET['id'], FILTER_VALIDATE_INT)) {
    http_response_code(400); die('Geçersiz ID.');
}
...
if (!$document) {
    http_response_code(404); die('Evrak bulunamadı.');
}

// Multi-tenant dosya yolunu doğru şekilde oluştur
$filename = $document['file_path'];
$file_path = __DIR__ . '/../uploads/' . $factory_id . '/documents/' . $filename;

if (!file_exists($file_path) || !is_readable($file_path)) {
    http_response_code(404); die('Dosya sunucuda bulunamadı veya okunamıyor.');
}
